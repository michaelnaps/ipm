...
function [u, C, n, brk] = newtons(P, dt, q0, u0, um, c, m, L, Cq, qd, eps)
    %% Setup - Initial Guess, Cost, Gradient, and Hessian
    N = length(um);
    uc = u0;
    Cc = cost(P, dt, q0, u0, uc, c, m, L, Cq, qd, " NN Initial Cost ");
    un = uc;  Cn = Cc;
    ...
    while (Cc > eps)
        ...
        g = cost_gradient(P, dt, q0, u0, uc, c, m, L, Cq, qd, 1e-3, " NN Main Loop Gradient ");
        H = cost_hessian(P, dt, q0, u0, uc, c, m, L, Cq, qd, 1e-3, " NN Main Loop Hessian ");

        % calculate and add the next Newton's step
        un = uc - H\g;

        % compute new values for cost, gradient, and hessian
        Cn = cost(P, dt, q0, u0, un, c, m, L, Cq, qd, " NN Main Loop Cost ");
        udn = abs(un - uc);
        count = count + 1;
